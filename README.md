...
- VS Code extension: preview (`0.1.3`). Published on both the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=julienblanchon.mujoco-viewer) and [Open VSX](https://open-vsx.org/extension/julienblanchon/mujoco-viewer) so it installs on VS Code, Cursor, Windsurf, VSCodium, Gitpod, code-server, and other forks.
