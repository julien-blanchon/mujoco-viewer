...
export interface ReflectorOptions {
	...
	/** How strong the reflection is (0.0 - 1.0) */
	mixStrength?: number;
}
...
export class Reflector extends THREE.Mesh {
	...
	constructor(geometry: THREE.BufferGeometry, options: ReflectorOptions = {}) {
		...
		const color =
			options.color !== undefined ? new THREE.Color(options.color) : new THREE.Color(0x7f7f7f);
		...
		(this as unknown as ReflectorMesh).onBeforeRender = (
			renderer: THREE.WebGLRenderer,
			scene: THREE.Scene,
			camera: THREE.Camera
		) => {
